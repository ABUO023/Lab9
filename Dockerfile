...
RUN cd /build && javac -cp /usr/local/tomcat/lib/servlet-api.jar *.java && cp *.class /usr/local/tomcat/webapps/ROOT/WEB-INF/classes/

COPY src/main/webapp/WEB-INF/web.xml /usr/local/tomcat/webapps/ROOT/WEB-INF/
COPY src/main/webapp/*.html /usr/local/tomcat/webapps/ROOT/
